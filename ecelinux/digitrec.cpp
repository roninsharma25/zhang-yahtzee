  ...
  if(otsu_mode){
    ...
    for (int j = 0; j < ROW_COUNT; j++) {
      ...
      for (int i = 3; i >= 0; i--) {
        ...
        printf("BEFORE CC \n");
        printf("NUM 4: %d \n", out_buffer(39,32).to_int());
        printf("NUM 3: %d \n", out_buffer(31,24).to_int());
        printf("NUM 2: %d \n", out_buffer(23,16).to_int());
        printf("NUM 1: %d \n", out_buffer(15,8).to_int());
        printf("NUM 0: %d \n", out_buffer(7,0).to_int());
        ...
        if ( (j >= 1) ) strm_out.write(out_buffer((COL)*8 + 7, COL*8));
        printf("NUM 4: %d \n", out_buffer(39,32).to_int());
        printf("NUM 3: %d \n", out_buffer(31,24).to_int());
        printf("NUM 2: %d \n", out_buffer(23,16).to_int());
        printf("NUM 1: %d \n", out_buffer(15,8).to_int());
        ...
        if (column_value >= COL) {
          row_value += 1;
          column_value = 0;
          //conn_comp_3rd_pass(out_buffer, COL, ROW);
        }
